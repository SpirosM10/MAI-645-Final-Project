...
class acLSTM(nn.Module):
    ...
    def calculate_loss_MSE(self, out_seq, groundtruth_seq):
        
        loss_function = nn.MSELoss()
        loss = loss_function(out_seq, groundtruth_seq)
        return loss

    def calcualte_loss_angle(self, out_seq, groundtruth_seq): #does not properly work

        cosine_err = torch.cos(out_seq - groundtruth_seq)
        loss = torch.sum(1-cosine_err)/(32*100)
        return loss
...
def train_one_iteraton(real_seq_np, model, optimizer, iteration, save_dance_folder, print_loss=False, save_bvh_motion=True):
    ...
    loss=model.calculate_loss_MSE(predict_seq, predict_groundtruth_seq)
    ...
    if(save_bvh_motion==True):
        ...
        read_bvh.euler_to_bvh(save_dance_folder+"%07d"%iteration+"_gt.bvh", gt_seq)
        read_bvh.euler_to_bvh(save_dance_folder+"%07d"%iteration+"_out.bvh", out_seq)
...
read_weight_path=""
write_weight_folder="../train_weight_aclstm_salsa_euler/"
write_bvh_motion_folder="../train_tmp_bvh_aclstm_salsa_euler/"
...
train(dances, dance_frame_rate, batch, 100, read_weight_path, write_weight_folder, write_bvh_motion_folder, 25001)
