...
#
    ...
    real_seq_dif_hip_x_z_np[:,:,Hip_index*3]=dif[:,:,Hip_index*3] #x axis only
    real_seq_dif_hip_x_z_np[:,:,Hip_index*3+2]=dif[:,:,Hip_index*3+2] # z axis
    ...
    if(save_bvh_motion==True):
        ...
        gt_seq=np.array(predict_groundtruth_seq[0].data.tolist()).reshape(-1,175)
        last_x=0.0
        last_z=0.0
        ...
        out_seq=np.array(predict_seq[0].data.tolist()).reshape(-1,175)
        last_x=0.0
        last_z=0.0
        ...
        #change this to match the representation
        read_bvh.write_quaternion_traindata_to_bvh(save_dance_folder+"%07d"%iteration+"_gt.bvh", gt_seq)
        read_bvh.write_quaternion_traindata_to_bvh(save_dance_folder+"%07d"%iteration+"_out.bvh", out_seq)
...
read_weight_path=""
write_weight_folder="../train_weight_aclstm_salsa_quaternions/"
write_bvh_motion_folder="../train_tmp_bvh_aclstm_salsa_quaternions/"
dances_folder = "../train_data_xyz/salsa_quaternions/"
dance_frame_rate=60
batch=32
