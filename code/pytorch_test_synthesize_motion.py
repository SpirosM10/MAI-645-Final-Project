...
class acLSTM(nn.Module):
    def __init__(self, in_frame_size=132, hidden_size=1024, out_frame_size=132):# change input and output frame size to match the representation
...
def calculate_loss(out_seq, groundtruth_seq):
...
def generate_seq(initial_seq_np, generate_frames_number, model, save_dance_folder):
    ...
    for b in range(batch):
        ...
        read_bvh.euler_to_bvh(save_dance_folder+"out"+"%02d"%b+".bvh", out_seq)
...
def load_dances_single(dance_folder):
    dance_files=os.listdir(dance_folder)
    dance_01 = dance_files[0]

    dance = np.load(dance_folder + dance_01)
    # dances=[]
...
def load_dances_test(dance_folder):
    dance_files=os.listdir(dance_folder)
    dances=[]
    for dance_file in dance_files:
        print ("load "+dance_file)
...
read_weight_path="../train_weight_aclstm_salsa_euler/0009000.weight"
write_bvh_motion_folder="../test_bvh_aclstm_euler_qualitative/"
dances_folder = "../train_data_xyz/salsa_euler/"
dance_frame_rate=60
batch=1
initial_seq_len=15
...
for_quantitative = True #change this

if for_quantitative:
    dances= load_dances_single(dances_folder)

else :
    dances = load_dances_test(dances_folder)


##Run this file with all the different representations, makin gsure to change all directories including for the quantitative evaluation
test(dances, dance_frame_rate, batch, initial_seq_len, generate_frames_number, read_weight_path, write_bvh_motion_folder)


# #Quantitative evaluation
# gt_directory= 'C:/Users/User/PycharmProjects/pythonProject1/Auto_Conditioned_RNN_motion-master/train_data_bvh/salsa/01.bvh'
#
# out_directory_euler = 'C:/Users/User/PycharmProjects/pythonProject1/Auto_Conditioned_RNN_motion-master/test_bvh_aclstm_salsa_euler_test/out00.bvh'
# out_directory_qua = 'C:/Users/User/PycharmProjects/pythonProject1/Auto_Conditioned_RNN_motion-master/test_bvh_aclstm_salsa_quaternions_test/out00.bvh'
# out_directory_6d = 'C:/Users/User/PycharmProjects/pythonProject1/Auto_Conditioned_RNN_motion-master/test_bvh_aclstm_salsa_6d_test/out00.bvh'
# out_directory_euler = 'C:/Users/User/PycharmProjects/pythonProject1/Auto_Conditioned_RNN_motion-master/test_bvh_aclstm_salsa_positional_test/out00.bvh'
#
# gt = read_bvh.get_train_data(gt_directory)
#
# out_posi = read_bvh.get_train_data(out_directory_euler)
# out_euler = read_bvh.get_train_data(out_directory_euler)
# out_qua = read_bvh.get_train_data(out_directory_qua)
# out_6d = read_bvh.get_train_data(out_directory_6d)
#
#
# def calculate_mse(out,gt,representation):
#     loss = np.sum((out[15:44] - gt[15:44])**2)/out.shape[0]
#     return print(f'{representation} loss = ',loss)
#
# # calculate_mse(out_euler,gt,'Euler Angle')
# # calculate_mse(out_qua,gt,'Quaternion')
# # calculate_mse(out_6d,gt,'6D Representation')
# calculate_mse(out_posi,gt,'Positional Representation')